...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE}/organizations/${user.organizationId}/dashboard`)
      .then(async response => {
  ...
  return <RequireAuth>
    <main className="shell">
      <div className="section-title"><span>لوحة التحكم</span><span className="status-pill good">MVP launch checklist</span></div>
      ...
      {summary && current && <>
        ...
        <div className="grid">
          ...
          <div className="item metric"><strong>Message usage</strong><p>{summary.usage ? `${summary.usage.messagesUsed}/${summary.usage.messageLimit} messages · remaining ${summary.usage.messagesRemaining}` : 'Usage not loaded yet'}</p>{summary.usage?.limitReached && <p>تم الوصول لحد الرسائل الشهري.</p>}</div>
          <div className="item metric"><strong>Channel usage</strong><p>{summary.usage ? `${summary.usage.channelsUsed}/${summary.usage.channelLimit} channels · remaining ${summary.usage.channelsRemaining}` : 'Usage not loaded yet'}</p>{summary.usage?.channelLimitReached && <p>تم الوصول لحد القنوات.</p>}</div>
          {summary.usage?.upgradeRecommendation && <div className="item metric"><strong>Recommended upgrade</strong><p>رشّحنا باقة {summary.usage.upgradeRecommendation.recommendedPlanName} بسعر {summary.usage.upgradeRecommendation.monthlyPriceEgp} EGP/شهر.</p><a className="btn" href={`/payment?organizationId=${summary.organization.id}&upgradePlanId=${summary.usage.upgradeRecommendation.recommendedPlanId}`}>Upgrade plan</a></div>}
        </div>
      </>}
    </main>
  </RequireAuth>;
